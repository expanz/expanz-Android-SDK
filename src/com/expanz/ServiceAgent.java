 ...
 *
 */
public  class ServiceAgent extends AsyncTask<RequestObject, Void, List<ResponseObject>> {
	 ...
	 * Constructor
	 * 
	 * @param context the activity/service that called this 
	 * @param callback
	 */
	public ServiceAgent(ServiceCallback callback) {
	...
	protected List<ResponseObject> doInBackground(RequestObject... requests) {
		...
		for(RequestObject<ResponseObject> request : requests) {
			...
			try {
				...
				Log.d(LOG_TAG + " request", requestPayload);
				post.setEntity(se);
			...
			try {
				...
				if (status != HttpStatus.SC_OK) {
					...
					Log.e(LOG_TAG + " response code", "server unable to service request " + status);
				...
				response.persist(responsePayload);
